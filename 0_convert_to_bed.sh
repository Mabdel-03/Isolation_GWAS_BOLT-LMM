...
if [ -s "${output_bfile}.bed" ] && [ -s "${output_bfile}.bim" ] && [ -s "${output_bfile}.fam" ] ; then
    ...
    # Update the genotype_bfile path in bolt_lmm.sh
    echo ""
    echo "IMPORTANT: Update the genotype_bfile path in bolt_lmm.sh to:"
    echo "  genotype_bfile=${output_bfile}"
...
echo ""
echo "Next steps:"
echo "1. Update genotype_bfile path in bolt_lmm.sh"
echo "2. Run 0_prepare_model_snps.sh to create model SNPs"
